...
function WallPlate(props) {
  const { pathname } = useLocation();
  const history = useHistory();

  const [categories, setCategories] = useState("Wall Plate");
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(WallPlate)
);
