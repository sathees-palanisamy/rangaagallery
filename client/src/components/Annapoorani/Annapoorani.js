...
function Annapoorani(props) {
  const { pathname } = useLocation();
  const history = useHistory();

  const [categories, setCategories] = useState("Annapoorani");
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Annapoorani)
);
