...
function Embossed(props) {
  const { pathname } = useLocation();
  const history = useHistory();

  const [categories, setCategories] = useState("Semi Embossed");
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Embossed)
);
