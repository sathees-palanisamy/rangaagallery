...
function Saraswathi(props) {
  const { pathname } = useLocation();
  const history = useHistory();

  const [categories, setCategories] = useState("Saraswathi");
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Saraswathi)
);
